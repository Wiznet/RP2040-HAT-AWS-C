/*
 * file					: http_interface.c
 * description	: Code for HTTP/HTTPS.
 * author				: Austin
 * company			: WIZnet
 * date					: 2021.05.27
 */
 ...
 * ----------------------------------------------------------------------------------------------------
 */
 /* HTTP/HTTPS connection informaion */
...
int32_t http_send_request(TransportInterface_t *pTransportInterface, uint8_t *buffer, char *pMethod, http_config_t *http_config)
{
	...
	requestHeaders.bufferLen = HTTP_BUF_SIZE;
	
	/* Initialize the response object. The same buffer used for storing
	 * request headers is reused here. */
	
	response.pBuffer = buffer;
	response.bufferLen = HTTP_BUF_SIZE;
    response.getTime = millis;
	
	httpStatus = HTTPClient_InitializeRequestHeaders(&requestHeaders, &requestInfo);
    if (httpStatus != HTTPSuccess) printf("Failed to initialize HTTP request headers: Error=%s.\n", HTTPClient_strerror(httpStatus));
    ...
    if (httpStatus == HTTPInsufficientMemory)
    {
        ...
        while(1)
        {
            //httpStatus = receiveHttpData(pTransportInterface, buffer, DATA_BUF_SIZE, &currentReceived);
            memset(buffer, NULL, HTTP_BUF_SIZE);
            currentReceived = pTransportInterface->recv(pTransportInterface->pNetworkContext, buffer, HTTP_BUF_SIZE);
...
int32_t http_get(uint8_t sock, uint8_t *buffer, char *http_url, tlsContext_t *tls_context)
{
	...
	if(g_http_config.http_state != HTTP_IDLE) return -1;
    ...
    memset(g_http_config.http_path, NULL, HTTP_DOMAIN_SIZE);
    memset(g_http_config.http_domain, NULL, HTTP_DOMAIN_SIZE);
...
int8_t https_connect(uint8_t sock, http_config_t *http_config, tlsContext_t *tls_context)
{
    ...
    //g_http_tls_context.clica_option = 0;
    //g_http_tls_context.rootca_option = MBEDTLS_SSL_VERIFY_NONE;
